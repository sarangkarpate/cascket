...
void *get_val_map(cass_map *m, char *column_name){
	return cfuhash_get(m->map, column_name);
}
int has_next(result_set *rs)
{
	...
	for (m = 0; m < rs->column_count; m++)
	{
		...
		else if (rs->col_desc_array[m].type == SET)
		{
			...
			x = uint8_to_int32(&rs->buffer[rs->curr_pos]); 
			rs->curr_pos += 4;
			rs->col_desc_array[m].cset->set = (void **) malloc(x*sizeof(void*));
			rs->col_desc_array[m].cset->length=x;
			for(j = 0; j < x; j++)
			{
				q = uint8_to_int32(&rs->buffer[rs->curr_pos]); 
				rs->curr_pos += 4;
				value = (char *)malloc(q + 1);
				uint8_to_string(&rs->buffer[rs->curr_pos], q, value);
				value[q] = '\0';
				rs->curr_pos+=q;
				rs->col_desc_array[m].cset->set[j] = value;
				cfuhash_put(rs->map, rs->col_desc_array[m].name, (void *)rs->col_desc_array[m].cset);
		...
		else if (rs->col_desc_array[m].type == MAP)//map
		{
			rs->col_desc_array[m].cmap->map = cfuhash_new();
			long long int  q;
			char *key ,*value;
			x = uint8_to_int32(&rs->buffer[rs->curr_pos]); 
			rs->curr_pos += 4;
			for (j = 0; j < x; j++)
			{
				...
				rs->curr_pos += 4;
				key = (char *)malloc(q + 1);
				uint8_to_string(&rs->buffer[rs->curr_pos], q, key);
				//			for (p = 0; p < q; p++, rs->curr_pos++)
				//			{
				//		printf("%x", rs->buffer[rs->curr_pos]);
				//			}
		...
		if (uint8_to_int32(&recvBuff[rs->curr_pos]) == ROWS) //Result of SELECT Query
		{
			...
			for (l = 0; l < COLUMN_COUNT; l++)
			{
				...
				else if (j == SET)//uint8 0x22
				{
					...
					rs->col_desc_array[l].cset = (cass_set*)malloc(sizeof(cass_set));
					j = uint8_to_int16(&recvBuff[rs->curr_pos]); 
					rs->curr_pos += 2;
					rs->col_desc_array[l].cset->type = j;
				}
				else if (j == MAP)//uint8 0x22
				{
					rs->col_desc_array[l].cmap = (cass_map *) malloc(sizeof(cass_map));
					j = uint8_to_int16(&recvBuff[rs->curr_pos]); /*this will describe the type of key in map*/
					rs->curr_pos += 2;
					rs->col_desc_array[l].cmap->key_type=j;
					j = uint8_to_int16(&recvBuff[rs->curr_pos]); /*this will describe the type of value in map*/
					rs->curr_pos += 2;
					rs->col_desc_array[l].cmap->value_type=j;
				}
				else
				{
					//printf("Type : %x\n",j);
